...
ColumnLayout {
    ...
    SplitButton {
        ...
        active: {
            if (Ai.useG4f) return _menuModel.find(it => it.isG4fGroup) ?? null
    ...
    SplitButton {
        ...
        active: _g4fMenuModel.find(it => it?.value === Ai.currentG4fModel)
             ?? (_g4fMenuModel[0] ?? null)
        ...
        Connections {
            ...
            function onItemSelected(item) {
                if (item) Ai.setG4fModel(item.value)
            }
        }
    ...
    readonly property string _mainLabel: {
        if (Ai.useG4f) return "Free (G4F)"
        return Ai.currentModel?.name ?? "Select model"
    }

    function _iconFor(model: var): string {
        const kid = model.keyId ?? ""
        if (kid === "gemini")     return "\uf6d7"   // sparkles
        if (kid === "anthropic")  return "\uf59f"   // brain
        if (kid === "openai")     return "\ufcbe"   // robot-face
        if (kid === "mistral")    return "\uebcb"   // wand
        if (kid === "deepseek")   return "\uebd2"   // flask
        if (kid === "groq")       return "\uef8e"   // cpu
        if (kid === "xai")        return "\uebdf"   // atom-2
        if (kid === "perplexity") return "\uf6d7"   // sparkles
        if (kid === "openrouter") return "\uea91"   // dots-circle
