...
// Ported from caelestia: M3 state layer with a Shape/RadialGradient press
// ripple (replaces the old implicitWidth-grow ripple). pShell's public API is
// preserved so existing consumers are untouched:
//   • `function onClicked()` overridable (not caelestia's `onClicked:` signal)
//   • `color` / `radius` settable (aliases to the base rect)
//   • `rect` alias (+ corner-radius sub-properties) for split/segmented buttons
//   • `showHoverBackground`, `disabled`
// CUtils.clamp is inlined via Math since pShell doesn't ship CUtils.
MouseArea {
    id: root
    ...
    property real stateOpacity: showHoverBackground && containsMouse ? 0.08 : 0
    ...
    // Overridable click handler (pShell contract).
    ...
    StyledRect {
        ...
        // Pick up radius from parent if it has one.
